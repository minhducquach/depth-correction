...
checkpoint = torch.load('/home/quachmd/Bureau/depth-correction/knowledge-distillation/src/checkpoints/mdm-distill-epoch=16-validation_loss=0.1362.ckpt')
...
num_tokens = dummy_tokens = torch.tensor([get_num_tokens()], dtype=torch.long)

output_onnx_file = "model.onnx"

torch.onnx.export(
    model.float(),
    (image_tensor, num_tokens, depth_tensor),
    output_onnx_file,
