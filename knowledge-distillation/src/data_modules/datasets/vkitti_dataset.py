...
class VirtualKitti(torch.utils.data.Dataset):
    def __init__(self, dir=BASE_DIR):
        self.color_paths = os.path.join(dir, 'vkitti_2.0.3_rgb')
        self.depth_paths = os.path.join(dir, 'vkitti_2.0.3_depth')

        search_pattern = os.path.join(self.color_paths, '**', '*.jpg')
        self.color_files = sorted(glob.glob(search_pattern, recursive=True))

        self.depth_files = []
        
        for color_path in self.color_files:
            depth_path = color_path.replace('vkitti_2.0.3_rgb', 'vkitti_2.0.3_depth')
            depth_path = depth_path.replace('/rgb/', '/depth/').replace('/rgb_', '/depth_').replace('.jpg', '.png')
